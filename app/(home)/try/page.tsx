...
export default function Try() {
  ...
  const [prestige, setPrestige] = useState('Way longer than it should be just so i can test what this is going to look like');
      ...
      <div className='flex items-center h-full w-full'>
        <div className='flex items-center flex-col w-1/2 min-h-[100px]'>
          ...
          <form onSubmit={() => handleSubmit()}>
            <input value={input} onChange={(e) => setInput(e.target.value)} className='border-2 rounded mt-4 px-2 h-[30px] hover:bg-gray-200 hover:border-pink-200' onBlur={() => handleSubmit()} />
          </form>
        </div>
        <div className='w-1/2 flex flex-col items-center min-h-[100px]'>
          ...
          <div id='input' className='text-xl rounded w-60 relative bg-white '>
            <div className='px-2 text-center whitespace-nowrap'>{translateTo || 'Who are you talking to?'}</div>
